...
const Header = props => {
	...
	const sun = <IconButton aria-label="expand">
			<SvgIcon>
		...
		moon = <IconButton aria-label="expand">
			<SvgIcon>
		...
		[ themeStyle, setThemeStyle ] = useState({border : "2px solid green"}),
		...
		handleChange = event => {
			...
			darkTheme === true
				? setThemeStyle({border : "2px solid red"})
				: setThemeStyle({border : "2px solid green"});
		...
		<header>
			<FormControlLabel
				...
				control={<Switch
					...
					name="changeTheme"
					style={themeStyle}
					inputProps={{"aria-label" : "change theme checkbox",
						role : "switch"}}
				/>}
				label="Choose theme"
			/>
